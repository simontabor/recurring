...
Account.prototype.updateBillingInfo = function(options, callback)
{
	var self = this;
	...
	binfo.update(options, function(err)
	{
		if (err)
			return callback(err);
...
Account.prototype.getAdjustments = function(callback)
{
	...
	RecurlyData.fetchAll(Adjustment, uri, function(err, payload)
	{
		if (err)
			return callback(err);
